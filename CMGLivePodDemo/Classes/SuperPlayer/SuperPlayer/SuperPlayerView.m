...
- (void)configTXPlayer {
    ...
    if (self.isLive) {
        ...
        config.connectRetryCount = 1.0f;
        config.connectRetryInterval = 1.0f;
...
- (void)moviePlayDidEnd {
    ...
    [self.netWatcher stopWatch];
    self.repeatBtn.hidden = YES;//self.repeatBtn.hidden = NO;
    self.repeatBackBtn.hidden = YES;//self.repeatBackBtn.hidden = NO;
    self.middleBlackBtn.hidden = YES;
...
- (void)onPlayEvent:(int)EvtID withParam:(NSDictionary *)param {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (EvtID == PLAY_EVT_PLAY_BEGIN || EvtID == PLAY_EVT_RCV_FIRST_I_FRAME) {
            ...
            if ([self.delegate respondsToSelector:@selector(superLivePlayerError:errCode:errMessage:)]) {
                [self.delegate superLivePlayerError:self errCode:EvtID errMessage:param[EVT_MSG]];
            }
        } else if (EvtID == PLAY_EVT_PLAY_LOADING){
            // 当缓冲是空的时候
            self.state = StateBuffering;
